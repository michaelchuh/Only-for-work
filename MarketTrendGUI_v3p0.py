...
# -*- coding: utf-8 -*-
"""
Market Trend Analyzer — v2.9 (cleanbase + red panel + light cascade)
- 精简级联：仅 Make → Model；其余维度独立，方便 fuel cell vs hybrid 等对比
- 移除黄色 hover tooltip（箭头/黄框），但保留 hover/click 触发右侧 Details
- Sales Trend 与 Market Structure 均恢复“红区”钻取（维度/度量/周期/Top-N/表格/条形图/导出CSV）
- Market Structure 增强：Metric(Share %/Units) + Group by + Filter
- “弹窗选择 + 搜索”只在 Make 维度提供（双栏，不丢选择）
"""
...
DIM_PRIORITY = [
    ...
    "Seats", "Data source", "Segment - Proton", "Drivetrain type"
]
...
def build_detail_panel(parent) -> Dict[str, object]:
    ...
    status = tk.StringVar(value=""); ttk.Label(card, textvariable=status, anchor="w").pack(fill=tk.X, pady=(6,0))
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Market Trend Analyzer — v2.9 (cleanbase)")
        self.geometry("1660x980"); self.minsize(1300, 780)
    ...
    def _build_ui(self):
        ...
        helpmenu.add_command(label="About", command=lambda: messagebox.showinfo("About","v2.9 cleanbase + red panel"))
    ...
    def _attach_hover(self, ax, lines, labels, unit_label, cb=None):
        ...
        try:
            ...
            def _(sel):
                # 隐藏注释
                try: sel.annotation.set_visible(False)
                except Exception: pass
                # 仍然执行回调 -> 更新右侧 Details
                x, _y = sel.target
                idx = int(round(x)); idx = max(0, min(idx, len(labels)-1))
    ...
    def _open_make_picker(self):
        if self.df is None or "Make" not in self.df.columns:
            messagebox.showinfo("Info","当前数据没有 'Make' 列。"); return
        all_makes = sorted(self.df["Make"].astype(str).dropna().unique().tolist(), key=lambda x: str(x))
        # 当前已选（忽略 All）
        cur_sel = self._get_selected(self.lb_trend["Make"]) or []
        selected = set(cur_sel)

        win = tk.Toplevel(self); win.title("选择 Make"); win.geometry("680x520")
        # 搜索
        tk.Label(win, text="搜索：").pack(anchor="w", padx=8, pady=(8,0))
        q = tk.StringVar(); ent = ttk.Entry(win, textvariable=q); ent.pack(fill=tk.X, padx=8)

        body = ttk.Frame(win); body.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)
        # 左：候选；右：已选
        left = ttk.Frame(body); right = ttk.Frame(body); left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True); right.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(8,0))

        ttk.Label(left, text="候选").pack(anchor="w")
        lb_all = tk.Listbox(left, selectmode=tk.EXTENDED, exportselection=False)
        sb1 = ttk.Scrollbar(left, orient="vertical", command=lb_all.yview); lb_all.configure(yscrollcommand=sb1.set)
        lb_all.pack(side=tk.LEFT, fill=tk.BOTH, expand=True); sb1.pack(side=tk.LEFT, fill=tk.Y)

        mid = ttk.Frame(body); mid.place_forget()  # 占位，不显示

        ttk.Label(right, text="已选择").pack(anchor="w")
        lb_sel = tk.Listbox(right, selectmode=tk.EXTENDED, exportselection=False)
        sb2 = ttk.Scrollbar(right, orient="vertical", command=lb_sel.yview); lb_sel.configure(yscrollcommand=sb2.set)
        lb_sel.pack(side=tk.LEFT, fill=tk.BOTH, expand=True); sb2.pack(side=tk.LEFT, fill=tk.Y)

        def refresh_all():
            lb_all.delete(0, tk.END)
            key = q.get().strip().lower()
            items = [v for v in all_makes if (key in v.lower())] if key else all_makes
            for v in items: lb_all.insert(tk.END, v)
        def refresh_sel():
            lb_sel.delete(0, tk.END)
            for v in sorted(selected): lb_sel.insert(tk.END, v)

        def add_from_all():
            picks = [lb_all.get(i) for i in lb_all.curselection()]
            for v in picks: selected.add(v)
            refresh_sel()
        def remove_from_sel():
            picks = [lb_sel.get(i) for i in lb_sel.curselection()]
            for v in picks:
                if v in selected: selected.remove(v)
            refresh_sel()
        def clear_sel():
            selected.clear(); refresh_sel()

        q.trace_add("write", lambda *_: refresh_all())
        refresh_all(); refresh_sel()

        btns = ttk.Frame(win); btns.pack(fill=tk.X, padx=8, pady=8)
        ttk.Button(btns, text="添加 →", command=add_from_all).pack(side=tk.LEFT)
        ttk.Button(btns, text="← 移除", command=remove_from_sel).pack(side=tk.LEFT, padx=6)
        ttk.Button(btns, text="清空", command=clear_sel).pack(side=tk.LEFT, padx=6)

        def on_ok():
            lb = self.lb_trend["Make"]
            lb.selection_clear(0, tk.END)
            if not selected:
                lb.selection_set(0)  # All
            else:
                # 遍历并选中
                for i in range(lb.size()):
                    if i == 0: continue
                    if lb.get(i) in selected:
                        lb.selection_set(i)
            self._on_cascade_change_for("Make")
            win.destroy()
        ttk.Button(btns, text="确定", command=on_ok).pack(side=tk.RIGHT)
        ttk.Button(btns, text="取消", command=win.destroy).pack(side=tk.RIGHT, padx=6)

    # ---------- Tab 1: Sales Trend ----------
    def _build_trend(self, root):
        pw = ttk.PanedWindow(root, orient=tk.HORIZONTAL); pw.pack(fill=tk.BOTH, expand=True)
        left = ttk.Frame(pw, width=360); center = ttk.Frame(pw); right = ttk.Frame(pw, width=360)
        pw.add(left, weight=2); pw.add(center, weight=6); pw.add(right, weight=3)

        def mk_box(label, extra_make_picker=False):
            frame = ttk.LabelFrame(left, text=label, padding=6)
            if extra_make_picker:
                # Make 用普通列表 + “选择…” 按钮（弹窗搜索不丢选择）
                top = ttk.Frame(frame); top.grid(row=0, column=0, columnspan=2, sticky="ew")
                ttk.Button(top, text="选择…", command=self._open_make_picker).pack(anchor="e")
                row = 1
            else:
                row = 0
            ...
            lb.grid(row=row, column=0, sticky="nsew"); sb.grid(row=row, column=1, sticky="ns")
            frame.columnconfigure(0, weight=1); frame.rowconfigure(row, weight=1)
            return frame, lb
        ...
        for i,(lab,col) in enumerate(sections):
            f, lb = mk_box(lab, extra_make_picker=(col=="Make")); f.grid(row=i//2, column=i%2, sticky="nsew", padx=4, pady=4)
        ...
        for col in self.CASCADE_ORDER:
            if col in self.lb_trend:
                self.lb_trend[col].bind("<<ListboxSelect>>", lambda e, c=col: self._on_cascade_change_for(c))
    ...
    # --- cascade (Make -> Model) ---
    def _on_cascade_change_for(self, changed_col: str):
        if changed_col not in ("Make","Model") or self.df is None: return
        if changed_col == "Make":
            # 计算在 Make 选择下有效的 Model 值
            df = self.df.copy()
            sel_make = self._get_selected(self.lb_trend["Make"])
            if sel_make is not None:
                df = df[df["Make"].astype(str).isin(sel_make)]
            lb = self.lb_trend["Model"]
            vals = sorted(df["Model"].astype(str).dropna().unique().tolist())
            prev = self._get_selected(lb)
            lb.configure(state="normal"); lb.delete(0, tk.END); lb.insert(tk.END, "(All)")
            for v in vals: lb.insert(tk.END, v)
            if prev:
                for i in range(lb.size()):
                    if lb.get(i) in set(prev): lb.selection_set(i)
            else:
                lb.selection_set(0)

    # ---------- plotting for Sales Trend ----------
    def on_plot_trend(self):
        ...
        # 点击点 → Breakdown → 更新 red-panel ctx
        def on_pick(event):
    ...
    # ---------- Tab 2: Market Structure（增强 + 红区）
    def _build_structure(self, root):
        pw = ttk.PanedWindow(root, orient=tk.HORIZONTAL); pw.pack(fill=tk.BOTH, expand=True)
        left = ttk.Frame(pw, width=320); center = ttk.Frame(pw); right = ttk.Frame(pw, width=360)
        pw.add(left, weight=2); pw.add(center, weight=6); pw.add(right, weight=3)
        ...
        # 新增：Metric + Group by + Filter
        self.struct_metric = tk.StringVar(value="Share %")
        ttk.Label(left, text="Metric:").pack(anchor="w", pady=(8,0))
        ttk.Combobox(left, textvariable=self.struct_metric, state="readonly",
                     values=["Share %","Units"]).pack(fill=tk.X)

        self.struct_agg = tk.StringVar(value="Total")
        ttk.Label(left, text="Group by:").pack(anchor="w", pady=(8,0))
        cb_agg = ttk.Combobox(left, textvariable=self.struct_agg, state="readonly",
                     values=["Total","Country","Make","Segment - Proton","Vehicle type"])
        cb_agg.pack(fill=tk.X)
        ...
        ttk.Label(left, text="Start").pack(anchor="w", pady=(8,0))
        ...
        cb_agg.bind("<<ComboboxSelected>>", _refresh_struct_filter)
    ...
    def on_plot_structure(self):
        ...
        dim = self.struct_dim.get(); metric = self.struct_metric.get()
        ...
        # 计算：Units 或 Share%
        if metric == "Units":
            agg = df.groupby(dim)[months].sum(numeric_only=True).reset_index().rename(columns={dim:"Series"})
        else:
            if gcol is None:
                num = df.groupby(dim)[months].sum(numeric_only=True)
                denom = df[months].sum(axis=0, numeric_only=True).replace(0, np.nan)
                agg = (num.div(denom, axis=1) * 100.0).reset_index().rename(columns={dim:"Series"})
            else:
                num = df.groupby([gcol, dim])[months].sum(numeric_only=True)
                denom = df.groupby(gcol)[months].sum(numeric_only=True).replace(0, np.nan)
                share = num.div(denom).mul(100.0)
                agg = share.reset_index().rename(columns={dim:"Series", gcol:"_Group"})
                agg["Series"] = agg["_Group"].astype(str) + " — " + agg["Series"].astype(str)
                agg = agg.drop(columns=["_Group"])
        ...
        for _, r in agg.iterrows():
            ...
            lines.append(ln)
        self.ax_struct.set_title(f"{'Share %' if metric=='Share %' else 'Units'} by {dim} — {self.struct_agg.get()} (Filter: {self.struct_filter_val.get()})")
        self.ax_struct.set_xlabel("Month"); self.ax_struct.set_ylabel("%" if metric=="Share %" else "Units")
        ...
        def on_hover(series_label, idx):
            ...
            key = "Share % " if metric=="Share %" else "Units"
            push_detail(self.struct_detail, {"Series": series_label, "Month": months[idx], key: val})
        self._attach_hover(self.ax_struct, lines, months, "%" if metric=="Share %" else "Units", cb=on_hover)

        for ln in lines:
            try: ln.set_picker(True); ln.set_pickradius(8)
            except Exception: pass

        # 红区默认上下文（Structure 默认 Share %）
        self.struct_detail["set_ctx"]({"df": df, "months": months, "locks": {}, "period": months[-1] if months else None,
                                       "period_cols": [months[-1]] if months else [],
                                       "default_metric": "Share %" if metric=="Share %" else "Units",
                                       "child_dim": dim})
        ...
        def on_pick(event):
            ...
            series_label = event.artist.get_label()
            month = months[idx]
            br = agg[["Series", month]].sort_values(by=month, ascending=False).head(50)
            breakdown = br[["Series", month]].rename(columns={"Series": "Peer", month:"val"})
        ...
        latest = agg.sort_values(by=months[-1], ascending=False)[["Series", months[-1]]].head(5) if len(months)>0 else pd.DataFrame(columns=["Series"])
        if metric == "Share %":
            pieces = [f"{r.Series}: {float(r[months[-1]]):.1f}%" for _, r in latest.iterrows()] if len(latest)>0 else []
        else:
            pieces = [f"{r.Series}: {float(r[months[-1]]):,.0f}" for _, r in latest.iterrows()] if len(latest)>0 else []
        self.kpi_struct.set("Latest — " + " | ".join(pieces) if pieces else "Latest — (no data)")
    ...
    def load_file(self, path: str):
        ...
        for col_attr, lb in getattr(self, "lb_trend", {}).items():
            if col_attr in df.columns:
                ...
                for v in vals_sorted: lb.insert(tk.END, v)
                lb.selection_set(0)
            else:
