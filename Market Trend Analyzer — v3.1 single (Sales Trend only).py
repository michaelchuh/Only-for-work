...
# -*- coding: utf-8 -*-
"""
Market Trend Analyzer — v3.1 single (Sales Trend only)
- 保留：Sales Trend + 红区（维度/度量/周期/Top-N/表格或条形图/导出CSV）
- Make 采用“弹窗双栏 + 搜索”选择器（不丢已选项）
- 轻级联：Make → Model；其余维度可独立组合（便于 fuel cell vs hybrid 等对比）
- 移除黄色 hover 注释；但 hover/click 会更新右侧 Details
- Aggregate by 支持：Total / Make / Model / Country / Segment - Proton / Segment - Luxury /
  Body style (Proton) / Seats / Drivetrain type
"""
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Market Trend Analyzer — v3.1 (single)")
    ...
    def _build_ui(self):
        ...
        # ---- Filters
        def mk_box(label, make_picker=False):
            frame = ttk.LabelFrame(left, text=label, padding=6)
            row = 0
            if make_picker:
                ttk.Button(frame, text="选择…", command=self._open_make_picker).grid(row=row, column=0, columnspan=2, sticky="e")
                row += 1
        ...
        for i,(lab,col) in enumerate(sections):
            f, lb = mk_box(lab, make_picker=(col=="Make")); f.grid(row=i//2, column=i%2, sticky="nsew", padx=4, pady=4)
        ...
        ttk.Combobox(
            opts, textvariable=self.trend_agg, state="readonly",
            values=[
                "Total","Make","Model","Country",
                "Segment - Proton","Segment - Luxury",
                "Body style (Proton)","Seats","Drivetrain type"
            ]
    ...
    def _open_make_picker(self):
        ...
        q = tk.StringVar(); ttk.Entry(win, textvariable=q).pack(fill=tk.X, padx=8)
    ...
    def load_file(self, path: str):
        ...
        self.trend_start_cb["values"] = vals; self.trend_end_cb["values"] = vals
        if self.months:
            self.trend_start.set(self.months[0]); self.trend_end.set(self.months[-1])
