...
# -*- coding: utf-8 -*-
"""
Market Trend Analyzer — GUI (Tkinter)  (v2: robust month detection)
Author: Your AI Assistant

What's new in v2
----------------
• Much more tolerant month-column detection:
  - "Jan '22", "Jan-22", "Jan 22", "January 2022"
  - "2022 Jan", "2022-01", "2022/01", "202201"
  - Chinese style like "2022年01月"
  - Handles curly apostrophe (’), spaces, hyphens, slashes, dots.
• Still ignores columns like YTD/Total automatically.
"""
...
def _normalize_colname(x: Union[str, object]) -> str:
    if isinstance(x, pd.Timestamp):
        # Excel date in header -> format "MMM 'YY"
        return x.strftime("%b '%y")
    s = str(x).strip()
    # Replace curly apostrophes with normal apostrophe
    s = s.replace("’", "'").replace("‘", "'")
    # unify whitespace
    s = re.sub(r"\s+", " ", s)
    return s
...
def _parse_month_from_header(s: str) -> Optional[Tuple[int,int]]:
    """
    Try to parse a header into (year, month). Return None if not a month.
    ...
    # 1) "jan '22" / "jan-22" / "jan 22" / "january 2022"
    ...
    if m:
        ...
        yr  = int(m.group(2))
        if yr < 100: yr += 2000 if yr <= 79 else 1900  # assume 2000+ for 00-79; 1980s fallback
        return (yr, mon)

    # 2) "2022 jan" / "2022-january"
    ...
    # 7) Already a Timestamp-like string: try pandas to_datetime with day 1
    try:
...
def detect_month_columns(df: pd.DataFrame) -> List[str]:
    """Return a list of columns that look like months, in the original order."""
    months = []
    for c in df.columns:
        if _parse_month_from_header(c) is not None:
            months.append(c)
    return months
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Market Trend Analyzer — Sales Data Only (v2)")
    ...
    def on_plot_trend(self):
        ...
        for _, row in ser.iterrows():
            ...
            self.ax_trend.plot(x, y, marker="o", linewidth=1.8, markersize=3, label=str(row["Series"]))
        self.ax_trend.set_title(f"Sales {self.trend_metric.get()} — {self.trend_agg.get()}")
        self.ax_trend.set_xlabel("Month"); self.ax_trend.set_ylabel("Units" if self.trend_metric.get()=="Trend" else "%")
    ...
    def on_plot_structure(self):
        ...
        for _, row in share.iterrows():
            y = row[months].astype(float).values
            self.ax_struct.plot(x, y, marker="o", linewidth=1.6, markersize=3, label=str(row["Series"]))
        ...
        # snapshot KPI: latest month composition (top 5)
    ...
    def _build_comp(self, root):
        ...
        # when level changes, refresh list
    ...
    def on_plot_comp(self):
        ...
        for _, row in agg.iterrows():
            y = row[months].astype(float).values
            self.ax_comp.plot(x, y, marker="o", linewidth=1.6, markersize=3, label=str(row["Series"]))
        ...
        # simple summary: last month ranking
    ...
    def load_file(self, path: str):
        ...
        # fill competitor list initially
        self._refresh_comp_series()
    ...
    def on_show_headers(self):
        ...
        # show first 50 to avoid huge dialogs
        preview = "\n".join(cols[:50])
        messagebox.showinfo("Column headers (first 50)", preview)

    def on_about(self):
        messagebox.showinfo("About",
            "Market Trend Analyzer v2 — Sales Data Only\n\n"
            "• Robust month detection (Jan '22 / 2022-01 / 202201 / 2022年01月, etc.)\n"
            "• Sales Trend: Trend/YoY/MoM with filters and month range\n"
            "• Market Structure: Share by Drivetrain type / Body / Price band (if columns exist)\n"
            "• Competitor Compare: overlay by Model/Make/Segment\n"
