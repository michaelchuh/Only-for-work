...
- 增强：Make 即时搜索；Sales Trend 左侧级联筛选；两页签三分栏可拖拽
"""
...
class App(tk.Tk):
    def __init__(self):
        ...
        # 级联顺序（列名必须与数据列一致）
        self.FILTER_ORDER = ["Country","Data type","Vehicle type","Segment - Proton","Segment - Luxury","Data source",
                             "Make","Model","Body style (Proton)","Seats","Drivetrain type","Pure electric range (kms)","Price band"]
        self._build_ui()
    ...
    # ---------- Cascading helpers (Sales Trend) ----------
    def _trend_current_filters(self) -> Dict[str, Optional[List[str]]]:
        out: Dict[str, Optional[List[str]]] = {}
        for col, lb in getattr(self, "lb_trend", {}).items():
            vals = self._get_selected(lb)
            out[col] = vals  # None means (All)
        return out

    def _filtered_df_until(self, upto_col: str, include_current: bool = True) -> pd.DataFrame:
        if self.df is None:
            return pd.DataFrame()
        df = self.df.copy()
        if upto_col not in self.FILTER_ORDER:
            return df
        stop_index = self.FILTER_ORDER.index(upto_col) + (1 if include_current else 0)
        active_map = self._trend_current_filters()
        for col in self.FILTER_ORDER[:stop_index]:
            vals = active_map.get(col)
            if vals is None or col not in df.columns:
                continue
            df = df[df[col].astype(str).isin(vals)]
        return df

    def _set_listbox_options(self, col: str, values: List[str], keep_selection: bool = True):
        lb = self.lb_trend.get(col)
        if lb is None:
            return
        prev_selected = set(lb.get(i) for i in lb.curselection())
        lb.configure(state="normal")
        lb.delete(0, tk.END)
        lb.insert(tk.END, "(All)")
        uniq = sorted(set([str(v) for v in values if pd.notna(v)]))
        for v in uniq:
            lb.insert(tk.END, v)
        # update search cache if enabled
        if hasattr(self, "_trend_search_vars") and col in self._trend_search_vars:
            setattr(lb, "_all_items", uniq)
            # apply current search again
            q = self._trend_search_vars[col].get().strip().lower()
            if q:
                # rebuild with filter
                current_sel = set(lb.get(i) for i in lb.curselection())
                ...
                for i in range(lb.size()):
                    if lb.get(i) in current_sel:
                        lb.selection_set(i)
        # restore selection
        if keep_selection and prev_selected:
            for i in range(lb.size()):
                if lb.get(i) in prev_selected:
                    lb.selection_set(i)
        else:
            lb.selection_set(0)

    def _on_trend_filter_change(self, changed_col: str):
        if changed_col not in self.FILTER_ORDER:
            return
        try:
            start_idx = self.FILTER_ORDER.index(changed_col)
            base_df = self._filtered_df_until(changed_col, include_current=True)
            for col in self.FILTER_ORDER[start_idx+1:]:
                if col not in self.lb_trend or col not in base_df.columns:
                    continue
                vals = base_df[col].dropna().astype(str).unique().tolist()
                self._set_listbox_options(col, vals, keep_selection=False)
                # cascade deeper if downstream already has selection
                sel_vals = self._get_selected(self.lb_trend[col])
                if sel_vals is not None:
                    base_df = base_df[base_df[col].astype(str).isin(sel_vals)]
        except Exception:
            pass
    ...
    def _build_trend(self, root):
        # Three-pane resizable: filters | chart | details
        ...
        def mk_box(label, with_search=False):
            ...
            if with_search:
                ...
                entry.grid(row=row, column=0, columnspan=2, sticky="ew", pady=(0,4))
                row += 1
            ...
            if with_search:
                def _filter(*_):
                    all_items = getattr(lb, "_all_items", None)
                    if all_items is None:
                        return
                    q = search_var.get().strip().lower()
                    selected_vals = set(lb.get(i) for i in lb.curselection())
                    lb.delete(0, tk.END); lb.insert(tk.END, "(All)")
                    candidates = all_items if not q else [v for v in all_items if q in v.lower()]
                    for v in candidates:
                        lb.insert(tk.END, v)
                    for i in range(lb.size()):
                        if lb.get(i) in selected_vals:
                            lb.selection_set(i)
                search_var.trace_add("write", lambda *_: _filter())
            return frame, lb, search_var

        self.lb_trend: Dict[str, tk.Listbox] = {}; self._trend_widgets: Dict[str, ttk.LabelFrame] = {}
        self._trend_search_vars: Dict[str, tk.StringVar] = {}
        sections = [("Country","Country", False),("Data type","Data type", False),("Vehicle type","Vehicle type", False),
                    ("Segment","Segment - Proton", False),("Segment - Luxury (IDN only)","Segment - Luxury", False),
                    ("Data source","Data source", False),("Make","Make", True),("Model","Model", False),
                    ("Body style","Body style (Proton)", False),("Seats","Seats", False),("Drivetrain type","Drivetrain type", False),
                    ("Pure electric range (kms)","Pure electric range (kms)", False),("Price band","Price band", False)]
        for i,(lab,col,with_search) in enumerate(sections):
            f, lb, sv = mk_box(lab, with_search=with_search); f.grid(row=i//2, column=i%2, sticky="nsew", padx=4, pady=4)
            self.lb_trend[col]=lb; self._trend_widgets[col]=f
            if sv is not None: self._trend_search_vars[col]=sv
        for r in range((len(sections)+1)//2): left.rowconfigure(r, weight=1)
        for c in range(2): left.columnconfigure(c, weight=1)

        # Bind listbox select for cascading
        for _col,_lb in self.lb_trend.items():
            _lb.bind("<<ListboxSelect>>", lambda e, col=_col: self._on_trend_filter_change(col))
    ...
    def load_file(self, path: str):
        ...
        for col_attr, lb in getattr(self, "lb_trend", {}).items():
            if col_attr in df.columns:
                ...
                lb.selection_set(0)
                # cache all items for search-enabled lists
                if hasattr(self, "_trend_search_vars") and col_attr in self._trend_search_vars:
                    setattr(lb, "_all_items", vals_sorted)
            else:
                lb.delete(0, tk.END); lb.insert(tk.END, "(N/A)"); lb.configure(state="disabled")

        # 初始触发一次级联，确保下游选项受上游选择约束
        first_existing = next((c for c in self.FILTER_ORDER if c in getattr(self, "lb_trend", {})), None)
        if first_existing:
            self._on_trend_filter_change(first_existing)
