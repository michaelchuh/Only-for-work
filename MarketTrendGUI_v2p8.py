...
# -*- coding: utf-8 -*-
"""
Market Trend Analyzer — v2.8
- NEW: Right-side "Details" panel on **Sales Trend** and **Market Structure**, in addition to **Competitor Compare**.
  • Hover → shows Series, Period, Value
  • Click → adds MoM %, YoY %, Rolling 3/12 (Units: Sum; %: Avg) and Top breakdown
- Keeps: Time granularity + Country VS (Sales Trend), secondary filter (Market Structure), robust month detection.
"""
import os, re, sys
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Market Trend Analyzer — v2.8")
    ...
    def _build_ui(self):
        ...
        # Tab 1 — Sales Trend (with right details panel)
        ...
        # Tab 2 — Market Structure (with right details panel)
        ...
        # Tab 3 — Competitor Compare (with details panel)
    ...
    # ---------- Tab 1: Sales Trend (with details) ----------
    ...
    def _on_pick_trend(self, event):
        ...
        agg_col = self._trend_ctx["agg_col"]; gran = self._trend_ctx["gran"]; metric = self._trend_ctx["metric"]
        ...
        # compute MoM/YoY on periodized numbers
        # lags depend on granularity
        if gran == "Monthly":
            yoy_lag, prev_lag = 12, 1
        elif gran == "Quarterly":
            yoy_lag, prev_lag = 4, 1
        ...
        # rolling
        if ylabel == "Units":  # raw units metrics
            # need periodized base units for rolling sums
            ...
            # percent series → rolling average
        ...
        if agg_col == "Country":
            ...
            child_col = "Make" if "Make" in base_df.columns else None
        elif agg_col is None:  # Total
            child_col = "Make" if "Make" in base_df.columns else None

        if child_col is not None:
            # need to map period label back to monthly columns range
            # approximate: for granularity Monthly the label equals a month col;
            # for others, sum corresponding months.
            cols = []
            if gran == "Monthly":
                cols = [period]
            else:
                # derive months that map to this period
                yq = period
    ...
    # ---------- Tab 2: Market Structure (with details) ----------
    ...
    def _on_pick_structure(self, event):
        ...
        # parse group & dim name from label when gcol present
        ...
        # Breakdown: 同一组下该月份 dim 的 Top 排名（peer view）
        if gcol is None:
    ...
    def on_about(self):
        tips = [
            "Market Trend Analyzer — v2.8",
            "• Sales Trend/Market Structure: right-side Details panel（hover + click drill）",
            "• Sales Trend: Country VS + time granularity",
            "• Market Structure: Aggregate by + secondary filter",
            "• Competitor Compare: Details panel retained",
